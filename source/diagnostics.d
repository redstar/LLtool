...
import cmdline : warningsAreErrors;
import std.stdio;
import std.string;
import std.traits : isSomeChar;

void warning(Char, A...)(const(char)[] buffer, size_t pos, in Char[] msg, A args) if (isSomeChar!Char)
{
    warning(buffer, pos, 1, format(msg, args));
}

void warning(Char, A...)(const(char)[] buffer, size_t pos, size_t len, in Char[] msg, A args) if (isSomeChar!Char)
{
    warning(buffer, pos, len, format(msg, args));
}

void warning(const(char)[] buffer, size_t pos, size_t len, string msg)
{
    if (warningsAreErrors)
        error(buffer, pos, len, msg);
    else
        message!"Warning"(buffer, pos, len, msg);
}
...
{
    message!"Error"(buffer, pos, len, msg);
    errorEmitted = true;
}
...
private:
void message(string type)(const(char)[] buffer, size_t pos, size_t len, string msg)
{
    ...
    stderr.writefln("%s %d,%d: %s", type, info.line, info.ofs, msg);
}
